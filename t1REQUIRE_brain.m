...
%Inputs: T1w - t1 weighted image
%        WM - white matter segmentation
%        GM - gray matter segmentation
%        CSF - cerebrospinal fluid segmentation
%        infoT1 - structure including .MagneticFieldStrength,
%               .RepetitionTime, and .EchoTime
...
%     plot(lookup_table(:,1), lookup_table(:,2))
    ...
    sz = size(mprage);
    t1map_mprage = zeros(sz);
    for j = 1:sz(1)
        for k = 1:sz(2)
            for m = 1:sz(3)
                if mprage(j,k,m) >= min(lookup_table(:,2)) && mprage(j,k,m) <= max(lookup_table(:,2))
                    try
                        t1map_mprage(j,k,m) = lookup_table((lookup_table(:,2) == mprage(j,k,m)),1);
                    catch
                        diff = lookup_table(:,2) - mprage(j,k,m);
                        [~, ind] = min(abs(diff));
                        t1map_mprage(j,k,m) = lookup_table(ind,1);
                    end
                end
            end
        end
